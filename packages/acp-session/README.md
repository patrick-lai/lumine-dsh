...
The composer model picker reads host `session.models`. This package registers a catalog-only adapter at apply (`grok` / `claude` / `codex` / `cursor`; Grok seeded with live 1.0.5 gold grok-4.6 / grok-4.5) and writes `model/selection` in the Agent constructor — before host `setup` freezes `selectionFor().current` on `deepseek-official`. The official ACP child still starts at session create (NDJSON, no authenticate unless required) and refreshes that catalog from `models` / `modelState` / `_meta.x.ai/sessionConfig.options`. `session.selectModel` maps onto `session/set_config_option`. Generation stays on the child; `stream()` throws.
