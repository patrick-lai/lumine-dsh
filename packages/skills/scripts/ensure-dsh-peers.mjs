#!/usr/bin/env node
/**
 * Link DSH peers into this package's node_modules for `link:` installs.
...
if (ensure) {
  ...
  if (linked.length === 0) {
    console.error(
      'ensure-dsh-peers: no DSH peers found. Set DSH_HOME and DSH_PROFILE to an installed Harness profile, then re-run.',
    )
    process.exitCode = 1
  } else {
