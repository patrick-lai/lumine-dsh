/**
 * Make the DSH peers resolvable from this package directory. `link:` installs
 * load `@lumine/dsh-skills` from its real path, so Node never walks the DSH
...
export function dshModuleRoots(env: NodeJS.ProcessEnv = process.env, cwd = process.cwd()): string[] {
  ...
  for (const profileName of ['web', 'headless', 'sdk', 'acp']) {
    roots.push(join(home, 'profiles', profileName, 'node_modules'))
  }
...
/** Link missing DSH peers before the package dynamically imports its plugin. */
